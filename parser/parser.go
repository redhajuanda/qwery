...
type Parser interface {
	Parse(ctx context.Context, queryTemplate string, data map[string]any) (string, []any, error)
}
...
func (p *parser) Parse(ctx context.Context, queryTemplate string, data map[string]any, placeholder Placeholder, templateFuncMap map[string]any) (string, []any, error) {

	stdTemplateFuncMap := map[string]any{
		"DerefBool":          DerefBool,
		"IsTimeZero":         IsTimeZero,
		"IsTimeNotZero":      IsTimeNotZero,
		"sub":                func(x int) int { return x - 1 },
		"FullTextSearch":     FullTextSearch,
		"WildcardLikePrefix": WildcardLikePrefix,
		"WildcardLikeSuffix": WildcardLikeSuffix,
	}

	maps.Copy(stdTemplateFuncMap, templateFuncMap)

	// create a new parser with the default placeholder set to ?
	parser, err := tqla.New(tqla.WithPlaceHolder(placeholder), tqla.WithFuncMap(stdTemplateFuncMap))
	if err != nil {
...
func (p *parser) interpolateQuery(_ context.Context, query string, args ...any) (string, []any, error) {
